...
// 抖动退化检测参数
// 正常步行每步产生 4 次子相切换（L→MS→PO→SW→L），1步/秒时 3s 窗口内约 12 次；
// 阈值须远高于正常步速上限（2步/秒 × 3s × 4 = 24），设为 40 只有传感器剧烈抖动才触发。
const uint32_t PHASE4_UNSTABLE_WINDOW_MS    = 3000; // 抖动检测时间窗口（3秒）
const uint8_t  PHASE4_UNSTABLE_TRANS_THRESH = 40;   // 3秒内子相切换≥40次则触发退化（正常步行≤24）
