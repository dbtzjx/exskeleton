...
为方便上位机直接控制下位机逻辑（例如启用髋关节力矩闭环控制），本项目在标准协议之外定义了一个板级控制报文：
...
|---|---|---|
|DATA[0]|命令字节|0xD1|
|DATA[1]|目标转矩电流低字节|int16_t iq low|
|DATA[2]|目标转矩电流高字节|int16_t iq high|
|DATA[3-7]|保留|0x00|

说明：收到该报文后，下位机将开启髋关节的力矩闭环控制模式，并以指定的 iq 值周期性（本固件实现默认 50ms）向对应电机发送 `0xA1` 转矩闭环控制命令。
...
说明：收到该报文后，下位机将停止周期性发送转矩命令，髋电机恢复为上层逻辑控制模式。

注意：这些板级扩展命令是本项目自定义的协议扩展，不属于驱动器原始协议的一部分。使用前请确保总线中其他设备不会与 0x200 冲突。